...
Design Docs generation (implemented internally as System Design generation), cost tracking, and the eval harness. These docs describe the system as
...
- [System Design Generation](system-design-generation.md) — internal implementation doc for Design Docs generation, covering per-kind lifecycle (lease, cache probe, gateway call, quality gate, persist, telemetry), `(repositoryId, kind, alignedImportCommitSha, provider, model, promptVersion)` idempotency key, and stale-job auto-resume that skips already-cached kinds.
- [Cost Tracking](cost-tracking.md) — gateway-boundary capture, per-message and per-kind persistence, raw `getUserCostBreakdown` aggregation, idempotent sharded viewer usage rollups across chat and Design Docs generation, and conservative `undefined`-on-miss semantics so pricing gaps surface as "cost unknown" instead of zero.
...
with Design Docs generation" subsection cross-references system-design-generation.md.
