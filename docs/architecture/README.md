...
- [Cost Tracking](cost-tracking.md) — gateway-boundary capture, per-message and per-kind persistence, raw `getUserCostBreakdown` aggregation, idempotent sharded viewer usage rollups across chat and System Design, and conservative `undefined`-on-miss semantics so pricing gaps surface as "cost unknown" instead of zero.
