...
- **localStorage**: Device conditions, workspace state, and frequent UI state that doesn't benefit from cross-device sync
...
| `systify.library.tabs.{wsId}` | Workspace state | localStorage | IDE pattern — "which tabs are open on this machine right now" |
| `systify.library.askTabs.{wsId}` | Workspace state | localStorage | Same rationale |
...
| `systify.activeWorkspaceId` | User identity | localStorage (first-paint cache) + DB | "My current main project" — DB is source of truth (`userPreferences.lastActiveWorkspaceId`) |
...
2. **Choose storage**:
   - Device condition / workspace state → localStorage
...
4. **Wire up cleanup**: For id-scoped keys (`prefix.{workspaceId|repoId}.…`), add the prefix to `WORKSPACE_SCOPED_PREFIXES` or `REPOSITORY_SCOPED_PREFIX` in `use-storage-gc.ts` — the hook will sweep orphans automatically whenever the live id set shrinks. For one-off, non-id-scoped cleanup (e.g., a schema change to a global key), call `removeKeysByPrefix()` directly from a migration entry point:
   ```ts
...
- When a **workspace** is deleted: `systify.library.tabs.{wsId}` and `systify.library.askTabs.{wsId}` keys are removed

Cleanup runs whenever the live id sets shrink (initial load, local deletion, or cross-tab deletion via Convex subscription). Adding a new id-scoped key? Add its prefix to `WORKSPACE_SCOPED_PREFIXES` or `REPOSITORY_SCOPED_PREFIX` in `use-storage-gc.ts` and the same machinery will sweep it.
...
- **Treating identity preferences as device-local.** "My current workspace" is identity, not device. Putting it in localStorage alone causes the well-known bug where a user switches workspaces on laptop A, opens laptop B, and lands somewhere stale. This is exactly why `systify.activeWorkspaceId` is a DB-backed value with localStorage as a first-paint cache only (see [`workspace-persistence-system-design.md`](./workspace-persistence-system-design.md)).
...
- **Forgetting orphan cleanup for id-scoped keys.** Any new `prefix.{workspaceId|repoId}.…` key must be reachable by `useStorageGC` or it will accumulate in users' browsers.
