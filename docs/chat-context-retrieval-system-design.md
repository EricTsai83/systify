...
Chat (specifically the `sandbox` mode, which is the only mode that loads code chunks; `discuss` skips repo lookups and `docs` is artifact-only) previously loaded the first 80 `repoChunks` from:
...
The new retrieval path gives `sandbox`-mode chat a better fit for the current Systify architecture:
