...
- A sandbox is provisioned only when live source work starts: a Sandbox-grounded Discuss reply prepares it inside the reply action, and Design Docs generation prepares it inside `systemDesignNode.runSystemDesignGeneration` before selected templates run.
