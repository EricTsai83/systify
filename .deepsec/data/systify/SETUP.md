# Agent setup for `systify`

This is a deepsec scanning workspace. Project `systify` was just registered
(target: `..`). Setup is incomplete — `data/systify/INFO.md`
...
2. **Fill in `data/systify/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id systify
pnpm deepsec process --project-id systify
```
