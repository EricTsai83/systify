...
describe("estimateCostUsd", () => {
  ...
  test("OpenAI gpt-5 sandbox tier prices correctly", () => {
    // 1M input @ $1.25 + 1M output @ $10 = $11.25
    expect(estimateCostUsd("openai", "gpt-5", { inputTokens: 1_000_000, outputTokens: 1_000_000 })).toBeCloseTo(11.25);
  });

  test("OpenAI gpt-5-mini discuss / library tier prices correctly", () => {
    // 1M input @ $0.25 + 1M output @ $2 = $2.25
    expect(estimateCostUsd("openai", "gpt-5-mini", { inputTokens: 1_000_000, outputTokens: 1_000_000 })).toBeCloseTo(
      2.25,
    );
  });

  test("Anthropic Opus prices correctly", () => {
  ...
  test("cachedInputTokens are billed at the cache-read tier (Anthropic)", () => {
  ...
  test("cacheWriteTokens are billed at the premium write tier (Anthropic Opus)", () => {
    // 1M cache write @ $18.75 + 0M input + 0M output = $18.75
    expect(
  ...
  test("reasoningTokens default to the output rate when no explicit reasoning tier", () => {
    // gpt-5 has no reasoningPerMillion → reasoning bills at output ($10/M)
    // 100k reasoning @ $10/M = $1.00
    expect(
      estimateCostUsd("openai", "gpt-5", { inputTokens: 0, outputTokens: 0, reasoningTokens: 100_000 }),
    ).toBeCloseTo(1);
  });
