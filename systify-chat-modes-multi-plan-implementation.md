...
| 09  | Daytona 失效的明確錯誤路徑                            | 1.5d | 04, 06, 08 | 同                         |
...
| 14  | Mode Examples + Auto-Suggest                  | 1.5d | 13     | 100%（公開）                  |
...
# Plan 09 — Daytona 失效的明確錯誤路徑

## 目標

Daytona 整體掛掉、單一 sandbox 突然 archived、quota 用完時，**讓失敗清晰可見並把決策權交還使用者**——不嘗試靜默用 docs 答案頂替 sandbox 答案。同時把「服務級 outage」與「單一 sandbox 失效」分流，避免 ops 級事件被使用者級事件淹沒。

## 設計原則（為什麼不自動降級到 docs）

1. **Silent quality drop**：使用者選 sandbox 是因為 docs 不夠；自動降級給的就是不夠的答案，但帶 `[A1]` 引用看起來很可信，使用者很可能直接套用
2. **Mid-session partial-degraded 無法收尾**：LLM 累積 3 個 read_file 結果、第 4 個失敗時，「以已收集結果繼續」會 produce 半驗證/半 hallucinate 的答案——citation lint（Plan 11）擋不到合法格式但外推的 `[path:line]`
3. **訊號被稀釋**：Daytona 整體掛（ops 該被 page）跟單一 sandbox archived（使用者可重啟）混成同一個 metric，故障診斷困難

## Done 標準（產品狀態）

- **單次 transient 失敗**（network blip、5xx、socket reset）→ 工具層自動 retry 一次（200ms 後）
- **Retry 仍失敗** → assistant message status 改 `failed`、`failureReason` 寫入分類好的 errorCode；partial trace 沿用 Plan 06 的 fold/drain 收尾為 interrupted
- **UI 顯示具體可操作錯誤**，依 errorCode 提供不同 action：
  - `sandbox_offline`（sandbox state ≠ ready，例如 archived） → `[Restart sandbox]` `[Retry]` `[Ask in Docs mode]`
  - `daytona_unreachable`（連線級失敗） → `[Retry]`，breaker 開啟時顯示 cooldown 倒數
  - `tool_timeout` / `path_not_found` / `tool_internal_error` → `[Retry]`
- **`[Ask in Docs mode]` 是 user-initiated**：按下才把原問題塞回 composer + 切到 docs mode（**不**自動送出，讓使用者確認）
- **Workspace-wide circuit breaker**：同 workspace 在 `SANDBOX_OUTAGE_WINDOW_SECONDS`（預設 60）內累積 `SANDBOX_OUTAGE_FAIL_THRESHOLD`（預設 5）次 `daytona_unreachable` 級失敗 → sandbox mode 暫時從 `availableModes` 移除（disabledReason `service_outage`），breaker 在 `SANDBOX_OUTAGE_COOLDOWN_SECONDS`（預設 300）後 half-open，下一個 sandbox 請求作為 probe，成功則 close、失敗則重 open
- **Mid-session 失敗不嘗試延續**：失敗在 tool boundary、訊息收尾為 failed；使用者按 Retry 才重新生成
- 失敗事件分類進 metrics（見「與其他 plan 相互影響」節）
...
| 檔案                                       | 變更                                                                                                                                                                  |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `env.ts`                                 | 新增 `SANDBOX_OUTAGE_WINDOW_SECONDS`、`SANDBOX_OUTAGE_FAIL_THRESHOLD`、`SANDBOX_OUTAGE_COOLDOWN_SECONDS`                                                                |
| `convex/schema.ts`                       | 新表 `daytonaCircuitBreaker`（單列 doc）：`{ scope: "global", state: "closed" \| "open" \| "half_open", openedAt?, halfOpenAt?, recentFailures: Array<{ at: number; reason: string }> }` |
| `convex/chat/circuitBreaker.ts`          | **新檔**：`recordDaytonaFailure(reason)`、`recordDaytonaSuccess()`、`getDaytonaBreakerState()`，含 sliding window prune 與半開 probe OCC                                      |
| `convex/chat/circuitBreaker.test.ts`     | **新檔**：closed → open → half_open → closed/open 狀態轉移                                                                                                                  |
| `convex/chat/sandboxTools.ts`            | 保留 retry-once；catch 後 map 到固定 errorCode；連線級失敗呼叫 `recordDaytonaFailure`、成功呼叫 `recordDaytonaSuccess`                                                                  |
| `convex/chat/generation.ts`              | tool 連續失敗 → 收尾 message 為 `failed` + `failureReason`；**移除**任何 degraded prompt / docs fallback 路徑                                                                     |
| `convex/chat/streaming.ts`               | 擴充既有 fail 函式接受 `failureReason`；確保 partial trace fold 走 Plan 06 helper                                                                                               |
| `convex/chatModeResolver.ts`             | `availableModes` 計算加 `getDaytonaBreakerState()` 檢查；open 時加 `service_outage` disabledReason 與 cooldown 倒數                                                            |
| `src/components/chat-failure-actions.tsx` | **新檔**：依 `failureReason` errorCode 渲染 action 按鈕；`[Restart sandbox]` 走既有 sandbox provision 入口                                                                        |
| `src/components/chat-panel.tsx`          | failed assistant message 下方渲染 `<ChatFailureActions>`                                                                                                                |
| `convex/chat-generation.test.ts`         | 改寫：第二次失敗後 status === failed、failureReason 正確、breaker 計數                                                                                                            |


## 工作項目

1. **Retry 策略**：transient（ETIMEDOUT、ECONNRESET、Daytona 5xx、fetch network error）retry 一次、間隔 200ms；non-transient（4xx、auth、path validation）直接 fail
2. **Error classification**（`sandboxTools.ts`）：catch 後對應到固定 errorCode：
   - `sandbox_offline` — sandbox state ≠ `ready`
   - `daytona_unreachable` — fetch failed / connection refused / Daytona 5xx
   - `tool_timeout` — 個別 tool 執行超時
   - `path_not_found` / `path_outside_repo` — 路徑問題
   - `tool_internal_error` — fallback
3. **Message 收尾**：retry 後仍失敗 → finalize message status `failed` + `failureReason: errorCode`、partial trace 沿用 Plan 06 的 `foldAndDrainToolCallEvents` 收尾。**不**改 prompt 重跑、**不**自動切到 docs。
4. **Circuit breaker**（`circuitBreaker.ts`）：
   - 只有 `daytona_unreachable` 通報；個別 sandbox archived **不**算 outage（使用者重啟即可）
   - sliding window：`recentFailures` 保留近 `SANDBOX_OUTAGE_WINDOW_SECONDS` 內事件，每次 record 順手 prune
   - 三狀態：closed → open（達 threshold） → half_open（cooldown 過後） → closed（probe 成功）/ open（probe 失敗）
   - half_open 期間最多一個 in-flight sandbox 請求作為 probe（用 OCC：對 doc revision 做 conditional update）
5. **Mode resolver 整合**（`chatModeResolver.ts`）：在現有 flag/allowlist/quota 檢查之後加 breaker check；disabledReason `service_outage`，文案「Sandbox service is temporarily unavailable. Try again in ~Ns.」（N 從 breaker state 算）
6. **UI failure actions**（`chat-failure-actions.tsx`）：
   - `sandbox_offline` → `[Restart sandbox]` / `[Retry]` / `[Ask in Docs mode]`
   - `daytona_unreachable` → `[Retry]`，breaker open 時加 cooldown 倒數
   - 其他 → `[Retry]`
   - `[Ask in Docs mode]` 行為：原 user message 內容塞回 composer、mode 切到 docs、autofocus，**不**自動送出
7. **Mid-session partial trace**：與 Plan 06 設計一致——fail 路徑也跑 fold/drain，partial trace 顯示為 `endedAt === startedAt` 的 interrupted entry，UI 沿用 Plan 06 的終態渲染

## Verification

- `bun test convex/chat/circuitBreaker.test.ts convex/chat-generation.test.ts convex/chatModeResolver.test.ts`
- 手動 1（單一 sandbox 失效）：mid-session 把 sandbox 手動 archive，下個 tool call 應 fail with `sandbox_offline`、UI 顯示 `[Restart sandbox]` 三選項
- 手動 2（Daytona outage）：把 `DAYTONA_API_URL` 改錯，連續發 5 個 sandbox 訊息，第 6 次起 sandbox 在 mode selector 應為 disabled、tooltip 顯示 outage + cooldown 倒數
- 手動 3（mid-session 不延續）：mid-session 第 4 個 read_file 失敗時，確認 message 不會「以前 3 個結果繼續答」，而是 fail 在第 4 個 tool 邊界

## 依賴

- Plan 04（sandbox flow + mode resolver 結構）
- Plan 06（partial trace fold/drain helper）
- Plan 08（完整工具集才能完整測 errorCode 分類）

## 與其他 plan 相互影響

- **Plan 13（metrics）**：metric 名稱由舊 Plan 09 的 `sandbox_session.fallback_to_docs_total` 改為 `sandbox_session.failed_total{reason}` + `sandbox_circuit_breaker.{opened,closed}_total`；rollout abort 條件以 `failed_total{reason="daytona_unreachable"}` 為主信號（Plan 13 的 metrics 章節屆時引用此處定義即可，不需另立）
- **Plan 14（auto-suggest）**：post-failure 的「切到 docs」由本 plan 失敗 UI 提供（user-initiated `[Ask in Docs mode]` 按鈕），**不**進 Plan 14 的 auto-suggest heuristic，避免兩處邏輯重疊與閃爍
- **Plan 10（cost cap）**：失敗於 tool boundary 的 message 仍照既有 finalize 路徑記錄已產生的 token / tool cost，不需特例；breaker open 期間 sandbox mode 直接 disabled，不會發起新請求
...
# Plan 14 — Mode Examples + Auto-Suggest

## 目標

最後一哩 UX：空 thread 顯示 example prompts 降低使用門檻；輸入時 heuristic 建議切到更合適的模式。**不**用 popover 攔截 mode 切換——mode switch 是高頻動作，modal 等級的 UI 在這裡會煩多於幫；業界主流的 chat 產品（ChatGPT / Claude / Cursor / Linear）都是用「caption + 空狀態範例 + 可選的 (i) icon」做被動揭露，而不是首次切換時跳 modal。

## Done 標準（產品狀態）

- 開新 thread 或 thread 為空時，composer 上方依當前 mode 顯示 2-3 張 example prompt 卡片；點卡片把文字塞入 composer（**不**自動送出，讓使用者調整再送）
- 切換 mode 時範例同步換成該 mode 的範例
- mode selector 旁一個 `(i)` info icon，點開後 popover 並排顯示三模式的一行說明 + 各一個範例提問（user-initiated，**不**會主動跳）
...
- inline hint 可手動 dismiss 且本 session 不再顯示（用 React state，**不需** localStorage）
...
| 檔案                                     | 變更                                                                                                                              |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `src/components/chat-panel.tsx`        | 空 thread render `<ModeExamples>`；selector 旁加 `<ModeInfoPopover>`；輸入即時以 `(input, currentMode, availableModes)` 跑 `suggestMode` heuristic |
| `src/components/chat-modes.ts`         | `MODE_CATALOG` ownership；加 `examplePrompts`，並輸出 `MODE_EXAMPLES` / `MODE_INFO_ENTRIES` 供 UI 共用穩定 mode keys                                   |
| `src/components/mode-examples.tsx`     | **新檔**：依當前 mode 渲染 2-3 張 example 卡片，點擊塞入 composer（不送出）                                                                          |
| `src/components/mode-info-popover.tsx` | **新檔**：`(i)` icon + popover，user-initiated 三模式說明                                                                                 |
| `src/lib/suggest-mode.ts`              | **新檔**：`suggestMode(input, currentMode, availableModes)` 純函式；只依 `availableModes` 的穩定 mode keys 建議可切換模式，不從 `chat-panel.tsx` state 推斷 |
| `src/lib/suggest-mode.test.ts`         | **新檔**：heuristic 各 case 邏輯測試                                                                                                    |
| `src/components/chat-panel.test.tsx`   | 擴充 example 卡片、(i) popover、inline hint 渲染與 dismiss 測試                                                                            |


## 工作項目

1. `MODE_CATALOG` 由 `src/components/chat-modes.ts` 擁有並加 `examplePrompts: string[]` 欄位（每 mode 2-3 句），確保文案集中一處不散落
2. `<ModeExamples>` 在 `messages.length === 0` 時 render；點卡片走既有 composer `setValue` 介面，**不**送出，讓使用者保留調整空間
3. `suggestMode(input, currentMode, availableModes)` 純函式，輸入字串 + 當前 mode + 可用 mode keys，回 `{ suggested: ChatMode; key: string; reason: string } | null`；用 regex / startsWith 判斷，且只建議存在於 `availableModes` 的穩定 mode key
4. inline hint 沿用既有 `<AppNotice>` variant；dismiss 用 `useState<Set<string>>` 記哪些 hint key 被關掉，session 內生效（換 thread / reload 後重置——可以接受，因為 hint 本身不打斷流程）
5. `<ModeInfoPopover>` 用既有 shadcn `Popover` 元件，不引入新依賴

## Verification

- `bun test src/components/chat-panel.test.tsx src/lib/suggest-mode.test.ts`
- 手動 1：新 thread 開三種 mode，看 example 卡片切換；點卡片字串塞入 composer 但未送出
- 手動 2：docs 模式輸入「請看 convex/chat/send.ts 第 80 行」，看 inline hint 出現並可 [Switch]
- 手動 3：點 (i) icon，popover 顯示三模式說明，點外面關閉
...
| Q3  | Sandbox 失敗時的處置策略       | 明確錯誤 + 使用者主動選擇（不靜默降級到 docs）                          | 09         |
