...
| 檔案                                     | 變更                                                                                                                              |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `src/components/chat-panel.tsx`        | `MODE_CATALOG` 加 `examplePrompts`；空 thread render `<ModeExamples>`；selector 旁加 `<ModeInfoPopover>`；輸入即時跑 `suggestMode` heuristic |
...
| `src/lib/suggest-mode.ts`              | **新檔**：`suggestMode(input, currentMode)` 純函式                                                                                    |
...
1. `MODE_CATALOG` 加 `examplePrompts: string[]` 欄位（每 mode 2-3 句），確保文案集中一處不散落
2. `<ModeExamples>` 在 `messages.length === 0` 時 render；點卡片走既有 composer `setValue` 介面，**不**送出，讓使用者保留調整空間
3. `suggestMode(input, currentMode)` 純函式，輸入字串 + 當前 mode，回 `{ suggested: ChatMode | null; reason: string }`；用 regex / startsWith 判斷
