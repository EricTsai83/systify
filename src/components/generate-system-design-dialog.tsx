...
const STARTER_SET: readonly RepositoryGuideKind[] = ["readme_summary", "architecture_overview"];
...
export function GenerateSystemDesignDialog({
  ...
  // Design Docs are optional templates, not a required checklist. Start empty
  // so the user makes an explicit choice; helper buttons offer common sets.
  const [selected, setSelected] = useState<Set<RepositoryGuideKind>>(() => new Set());
  ...
  const selectStarterSet = () => {
    setSelected((prev) => new Set([...prev, ...STARTER_SET.filter((kind) => !queuedKinds.has(kind))]));
  };

  const selectAllTemplates = () => {
    setSelected(new Set(REPOSITORY_GUIDE_SECTIONS.map((item) => item.kind).filter((kind) => !queuedKinds.has(kind))));
  };
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {`Generate ${REPOSITORY_GUIDE_COPY.name}`}
          </DialogTitle>
          <DialogDescription>
            Choose optional templates to turn into repository-specific design docs. Generate only the docs that help
            this repository.
          </DialogDescription>
        </DialogHeader>
        ...
        {jobInProgress ? (
          <Alert>
            <AlertDescription className="text-[12px]">
              {REPOSITORY_GUIDE_COPY.name} generation is already running. Sections already in this run are locked;
              select more to generate them in the same queued work.
            </AlertDescription>
          </Alert>
        ) : null}
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button type="button" variant="outline" size="sm" onClick={selectStarterSet} disabled={isSubmitting}>
            Starter set
          </Button>
          <Button type="button" variant="outline" size="sm" onClick={selectAllTemplates} disabled={isSubmitting}>
            Select all
          </Button>
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col gap-0.5">
                    <div className="flex items-center gap-2">
                      <SectionIcon size={12} weight="bold" className="text-muted-foreground" />
        ...
        >
          ...
          <div className="flex flex-1 flex-col gap-0.5">
            ...
            <p className="text-[11px] leading-snug text-muted-foreground">
              Skip the cache and rebuild every selected design doc from scratch. Use when the repository's content
        ...
        {showCacheHint ? (
          <Alert className="border-emerald-600/30 bg-emerald-600/5 text-[12px] text-emerald-900 dark:text-emerald-200">
            <AlertDescription className="text-[12px]">
              {allCached
                ? `The selected ${cachedCount === 1 ? REPOSITORY_GUIDE_COPY.sectionName : REPOSITORY_GUIDE_COPY.sectionNamePlural} already exist for this commit and model — the run will be near-instant.`
                : `${cachedCount} selected ${cachedCount === 1 ? REPOSITORY_GUIDE_COPY.sectionName : REPOSITORY_GUIDE_COPY.sectionNamePlural} already exist for this commit and model. The run will reuse ${cachedCount === 1 ? "it" : "them"} and generate the remaining selected docs.`}
            </AlertDescription>
          </Alert>
        ) : null}
        ...
        <DialogFooter className="flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div className="text-[11px] text-muted-foreground">
            ...
            {selected.size === 1 ? REPOSITORY_GUIDE_COPY.sectionName : REPOSITORY_GUIDE_COPY.sectionNamePlural}.
          </div>
