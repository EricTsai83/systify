...
describe("SystemDesignStatusBanner", () => {
  ...
  test("maps model_empty_output to the empty-output reason text", () => {
    ...
    expect(screen.getByText(/The model didn't produce a complete design doc/)).toBeInTheDocument();
  });
  ...
  test("aggregates mixed reasons under the mixed-reason text", () => {
    ...
    setMutation(async () => undefined);
    render(<SystemDesignStatusBanner repositoryId={repositoryId} />);
    expect(screen.getByText("Couldn't generate 2 design docs")).toBeInTheDocument();
    expect(screen.getByText(/Some design docs couldn't be generated/)).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /Generate 2 design docs/ })).toBeInTheDocument();
  });
