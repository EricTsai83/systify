...
/* eslint-disable react-hooks/exhaustive-deps, @typescript-eslint/no-unused-vars -- shadcn-generated; branch + attachment helpers follow upstream ai-elements ergonomics */
...
export const MessageContent = ({ children, className, ...props }: MessageContentProps) => (
  // User bubble uses border + bg-secondary instead of upstream ai-elements'
  // rounded-lg so it conforms to the project's sharp / `rounded-none`
  // aesthetic (see `styles/tokens.css`). Background + border together still
  // distinguish it from the bg-background canvas.
  <div
    className={cn(
      "is-user:dark flex w-fit max-w-full min-w-0 flex-col gap-2 overflow-hidden text-sm",
      "group-[.is-user]:ml-auto group-[.is-user]:border group-[.is-user]:bg-secondary group-[.is-user]:px-3 group-[.is-user]:py-2 group-[.is-user]:text-foreground",
...
export const MessageBranchSelector = ({ className, from, ...props }: MessageBranchSelectorProps) => {
  ...
  // Upstream ai-elements re-rounds the join corners to `rounded-l-md` /
  // `rounded-r-md`, but our Button primitive is `rounded-none` so that
  // override would just leave one-sided partial rounding. Letting
  // ButtonGroup's flat-join classes pass through keeps the sharp aesthetic.
...
export function MessageAttachment({ data, className, onRemove, ...props }: MessageAttachmentProps) {
  ...
  return (
    <div className={cn("group relative size-24 overflow-hidden", className)} {...props}>
      {isImage ? (
        <>
          <img
