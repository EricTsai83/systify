...
export function useDefaultModelPick(args: {
  ...
  /**
   * `threads.lockedProvider`. When the thread is locked, the
   * capability default's provider may not match — in that case the
   * hook prefers the locked-provider capability default by reading
   * `listPickableModels({ provider, capability })` and returning the
  ...
  // Provider filter applied only when the thread is locked to a
  // provider whose capability default differs from the global pick.
  // Saves a query roundtrip for the common unlocked / matching case.
